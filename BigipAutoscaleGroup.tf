...
BigipAutoScalingInstanceProfile {
    {
        ...
        "Metadata": {
            "AWS::CloudFormation::Designer": {
                "id": "05d07a19-56ae-450e-a2a1-c420484868f1"
            }
        }
    }
}
...
resource "aws_autoscaling_group" "BigipAutoscaleGroup" {
  availability_zones        = ["us-east-1a"]
  name                      = "waf-${var.fqdn_app_name}-${var.deploymentName}-BigipAutoscaleGroup"
  max_size                  = var.scalingMaxSize
  min_size                  = var.scalingMinSize
  health_check_grace_period = 300
  health_check_type         = "ELB"
  force_delete              = true
  launch_configuration      = "${aws_launch_configuration.BigipLaunchConfig.name}"
}

resource "aws_launch_configuration" "BigipLaunchConfig" {
  name                        = "waf-${var.fqdn_app_name}-${var.deploymentName}-BigipLaunchConfig"
  image_id                    = "ami-04d6271456d114d6e"
  ...
  #  no_device   = "true"
  #}
  iam_instance_profile = aws_iam_instance_profile.BigipAutoScalingInstanceProfile.name
  enable_monitoring    = false
  security_groups      = [aws_security_group.bigipexternalsecuritygroup.id]
  user_data            = var.LaunchConfigUserData
}

resource "aws_iam_instance_profile" "BigipAutoScalingInstanceProfile" {
  name = "waf-${var.fqdn_app_name}-${var.deploymentName}-InstanceProfile"
  role = "${aws_iam_role.BigipAutoScalingAccessRole.name}"
}

resource "aws_iam_role" "BigipAutoScalingAccessRole" {
  name = "waf-${var.fqdn_app_name}-${var.deploymentName}-AccessRole"
  path = "/"
  #Policies
